...
train<-fread("./project/volume/data/interim/train_V3.csv")
test<-fread("./project/volume/data/interim/test_V3.csv")
...
#################################
# make a no current_price model #
#################################

sub_train<-as.matrix(train[,2:15])
gl_model<-cv.glmnet(sub_train, train_y, alpha = 1,family="gaussian",foldid = year_num,nfolds = length(unique(year_num)))
bestlam<-gl_model$lambda.min
gl_model<-glmnet(sub_train, train_y, alpha = 1,family="gaussian")
plot_glmnet(gl_model)
no_CP_coef<-predict(gl_model,type="coefficients",s=bestlam)
...
CP_coef<-predict(gl_model,type="coefficients",s=bestlam)


# compare no Current Price to with Current Price
