...
#########################################
# Demo non-statistical learning results #
#########################################

lm_train<-data.table(train)
lm_train$future_price<-train_y
BS_lm_fit<-lm(future_price~.,data=lm_train)
no_BS_lm_fit<-lm(future_price~.,data=lm_train[,c(1:13,514)])

summary(BS_lm_fit)
summary(no_BS_lm_fit)
...
coef_BS<-predict(gl_model,type="coefficients",s=bestlam)

########################################
# quickly compare to no BS in the data #
########################################
...
gl_model<-glmnet(train, train_y, alpha = 1,family="gaussian")
plot_glmnet(gl_model)
coef_no_BS<-predict(gl_model,type="coefficients",s=bestlam)
