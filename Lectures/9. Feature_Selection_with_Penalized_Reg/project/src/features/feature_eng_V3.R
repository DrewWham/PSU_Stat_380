...
# clean up power and toughness
card_tab$power[card_tab$power=="*"]<-NA
card_tab$toughness[card_tab$toughness=="*"]<-NA
card_tab$power[card_tab$power==""]<-NA
card_tab$toughness[card_tab$toughness==""]<-NA

card_tab$power<-as.numeric(card_tab$power)
card_tab$toughness<-as.numeric(card_tab$toughness)

master<-merge(master,card_tab[,.(id,rarity,Legendary,power,toughness)],all.x=T)
...
fwrite(train,"./project/volume/data/interim/train_V3.csv")
fwrite(test,"./project/volume/data/interim/test_V3.csv")
