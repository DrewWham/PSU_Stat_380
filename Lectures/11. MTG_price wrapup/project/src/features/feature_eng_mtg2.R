...
card_tab$Mythic<-0
card_tab$Mythic[grep("Mythic",card_tab$rarity)]<-1
...
master<-merge(master,card_tab[,.(id,rarity,Legendary,Mythic)],all.x=T)
master<-merge(master,types_tab,all.x=T)

master$current_price[is.na(master$current_price)]<-mean(master$current_price,na.rm=T)

master$Land_price_int<-master$Land*master$current_price
master$Legendary_price_int<-master$Legendary*master$current_price
master$Planeswalker_price_int<-master$Planeswalker*master$current_price
master$Mythic_price_int<-master$Mythic*master$current_price
